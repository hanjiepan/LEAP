"""
visual_example_fri_vs_clean.py: an example for source separation
...
if __name__ == '__main__':
    ...
    if script_purpose == 'testing':
        ...
        parameter_set = {
            ...
            'data_file_name':
                './data/ast_src_resolve/src_param_20170626-231925.npz',
            'mgain': 0.03,
            'precomputed_result_name':
                './result/ast_src_resolve/src_sep_plot_data_precomputed.npz',
            'dpi': 600,
            'marker_scale': 0.2,
            'cmap': truncate_colormap(plt.get_cmap('magma_r'), 0, 0.75),
            'threshold': None,
    ...
    separation = np.radians(1.5 / 60.)  # 1'30''
    ...
    data_file_name = \
        './data/BOOTES24_SB180-189.2ch8s_SIM_{num_sti}STI_146MHz_{num_station}Station_1Subband.npz'.format(
            num_sti=num_sti, num_station=num_station
        )
    # extract data
    ...
    tau_x = tau_y = np.radians(min(max(0.5, np.degrees(separation) * 3), FoV_degree)).squeeze()
    ...
    # save plotting data
    np.savez(result_dir + 'src_sep_plot_data.npz',
             x_ks=x_ks, y_ks=y_ks, alpha_ks=alpha_ks,
    ...
    bash_cmd = 'export PATH="/usr/bin:$PATH" && ' \
        ...
        auto_threshold=3,
        threshold=parameter_set['threshold'],
        imag_format='png',
    ...
    planar_plot_diracs_zoom_J2000(
        ...
        background_img=img_clean,
        marker_scale=parameter_set['marker_scale'],
        marker_alpha=0.6, save_fig=save_fig,
    ...
    if parameter_set['more_data']:
        # load LOFAR layout
        num_station = 56  # 56 <= one station is missing if use 57 -> not all correlations available
